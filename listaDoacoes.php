...
$sql_num_rows = "SELECT COUNT(iddoacao) as total FROM doacao WHERE iddoador = ?";
...
if (!empty($page) && $num_rows > 0){
    ...
    $sql = "SELECT * FROM doacao WHERE iddoador = $userId ORDER BY data_doacao DESC LIMIT $first_row, $max_rows_pg";
    ...
    while($data = $res->fetch(PDO::FETCH_ASSOC)){
        ...
        $sql_receiver = "SELECT nome_usuario FROM usuario WHERE idusuario = $idReceptor";
